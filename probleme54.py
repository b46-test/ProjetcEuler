mains=[["8C", "TS", "KC", "9H", "4S", "7D", "2S", "5D", "3S", "AC"],
...
["1D", "2D", "4C", "5C", "6C", "AH", "KH", "QH", "JH", "TH"]]

coul2val={"D":0,"C":1,"H":2,"S":3}
comb2val={"pair":8,"twopairs":7,"three":6,"straight":5,"flush":4,"full":3,"four":2,"straightflush":1,"royal":0}

def cart2val(carte):
	return fig2val(carte[0])*10+coul2val[carte[1]]
...
def main2val(main):
	vide=[0,100,100,10000,0,100,100,10000,100]
	score=[0,100,100,10000,0,100,100,10000,100]
	cartes=map(lambda x: x/10, main)
	couleurs=map(lambda x: x%10,main)
	
	diffcartes=[0,0,0,0]
	diffcouleurs=[0,0,0,0]
	for i in range(4):
		diffcartes[i]=cartes[i+1]-cartes[i]
		diffcouleurs[i]=couleurs[i+1]-couleurs[i]
	if(diffcouleurs==[0,0,0,0]):
		score[comb2val["flush"]]=1

	if(diffcartes==[1,1,1,1]):
		if(diffcouleurs==[0,0,0,0]):
			if(cartes[0]==10):
				score[comb2val["royal"]]=1
			else:
				score[comb2val["straightflush"]]+=cartes[0]
		else:
			score[comb2val["straight"]]+=cartes[0]
	elif(diffcartes[0:3]==[0,0,0]):
		score[comb2val["four"]]+=cartes[0]
	elif(diffcartes[1:4]==[0,0,0]):
		score[comb2val["four"]]+=cartes[0]
	elif(diffcartes.count(0)==3):
		if diffcartes[0:2]==[0,0]:
			score[comb2val["full"]]+=cartes[0]*100+cartes[4]
		else:
			score[comb2val["full"]]+=cartes[4]*100+cartes[0]
	
	elif(diffcartes[0:2]==[0,0] or diffcartes[1:3]==[0,0] or diffcartes[2:4]==[0,0]):
		score[comb2val["three"]]+=cartes[2]
	elif(diffcartes.count(0)==2):
		p=diffcartes.index(0)
		score[comb2val["twopairs"]]+=cartes[p]+cartes[diffcartes[p+1:].index(0)+p+1]*100
	elif(diffcartes.count(0)==1):
		score[comb2val["pair"]]+=cartes[diffcartes.index(0)]

	hauteurs=1000**7
	for i in range(5):
		hauteurs+=cartes[i]*(1000**i)

	for i in comb2val:
		if score[comb2val[i]]!=vide[comb2val[i]]:
			print i

	return int("".join(map(lambda x:str(x),score+[hauteurs])))


total=0

for main in mains:
	main=map(cart2val,main)
	main1=main[0:5]
	main2=main[5:10]
